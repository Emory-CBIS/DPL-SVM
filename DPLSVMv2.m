...
for jj=1:10 
    ...
    count1 = (jj-1)*(P/10)+1; count2 = jj*(P/10);
    Xtilde = X; Xtilde(:,count1:count2) = [];
    beta_tilde = beta; beta_tilde(count1:count2) = [];
    nXforbeta = sqrt(sigma_sq_epsilon)*diag(Yi./sqrt(rho))*X(:,count1:count2);
    nYforbeta = sqrt(sigma_sq_epsilon)*(rho+1 - diag(Yi)*Xtilde * beta_tilde)./sqrt(rho);
% toc(tstart)
    nXtX = nXforbeta' * nXforbeta;
    nXtY = nXforbeta' * nYforbeta;
       % posterior_variance = inv(nXtX + diag(1 ./ sigma_sq_beta));  

    %    modefied to use Woodbury matrix identity here
        Dinv = diag(sigma_sq_beta(count1:count2));
        sigma_beta_mat = sigma_sq_beta(count1:count2)*ones(1,N);
        nXforbeta_sigmabeta = sigma_beta_mat.*nXforbeta'; %faster way to compute Dinv*nXforbeta'
        posterior_variance = Dinv - (nXforbeta_sigmabeta)*inv(eye(N)+nXforbeta*nXforbeta_sigmabeta)*(nXforbeta_sigmabeta');
        posterior_variance = (posterior_variance + posterior_variance') ./ 2;
%         eig1 = min(eig(posterior_variance));
%         if eig1 < 0
%             posterior_variance = posterior_variance + eye(P)*(0.1-eig1);
%         end
        posterior_mean     = posterior_variance * (nXtY);
        % can be done more efficient if needed
        beta(count1:count2)            = mvnrnd(posterior_mean, posterior_variance);
